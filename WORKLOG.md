...
- **수정 파일:** index.html
- **변경 내용:**
  - STUDY-NOTES 최신 회차 아이디어를 반영해 프로토타입 검증용 접근성 체크 섹션에 즉시 테스트 블록을 추가.
  - 키보드 탭 순서를 바로 검증할 수 있도록 3단계 포커스 흐름(버튼→링크→버튼)과 미리보기 라벨을 배치.
  - 랜드마크 순서(header→main→region→footer) 확인 안내와 고대비 토글(contrast)을 추가해 초기 사용자 테스트 범위를 가볍게 확장.
- **출처:** STUDY-NOTES 최신 회차
- **다음 세션 예고:** 체크 섹션의 탭 순서와 고대비 토글이 스크린리더 랜드마크 탐색에서 예상대로 동작하는지 1회 스모크 점검.
---
